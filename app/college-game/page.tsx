...
async function getOrGenerateDailyAthletes() {
  ...
  if (docSnap.exists() && Array.isArray(docSnap.data().athletes)) {
    ...
    return docSnap.data().athletes.slice(0, MAX_LEVELS);
  } else {
    // You may want to implement random fallback logic here
    return [];
  }
}
...
async function recordCollegeGameScore(score: number, time: number, user: User | null) {
  try {
...
async function updateCollegeAverageScore(userId: string) {
  try {
...
async function fetchCollegeAverageScore(userId: string) {
  try {
    const docRef = doc(db, "collegeAverages", userId);
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
...
const CollegeGuess: React.FC = () => {
  ...
  // Load athletes for today
  useEffect(() => {
    const loadAthletes = async () => {
      const athletes = await getOrGenerateDailyAthletes();
  ...
  useEffect(() => {
    ...
    const fetchCloudAvg = async () => {
      const userId = getCurrentUserId(user);
      const avg = await fetchCollegeAverageScore(userId);
      setCloudAvg(avg);
    };
    fetchCloudAvg();
  }, [user, showStats]);
  ...
  const handleSubmitGuess = () => {
    if (!athletes.length || !athletes[currentLevel - 1]) return;
    const guessRaw = guessInputRef.current?.value.trim() || "";
    const guess = guessRaw.toLowerCase();
    const correctAnswer = athletes[currentLevel - 1].college.toLowerCase();
    setFeedback("");
    ...
    if (wordMatch) {
      ...
      if (distance <= 2 && guess.length > 0) {
        ...
        setFeedback(`Swing and a miss! The answer was ${athletes[currentLevel - 1].college}.`);
  ...
  const handleNextLevel = () => {
    if (currentLevel < MAX_LEVELS) {
      ...
      recordCollegeGameScore(score, elapsedTime, user);
    }
  };
    ...
    <div className="trivia-bg">
      ...
      <div className="trivia-card">
        ...
        <div className="trivia-question-text">
          {athletes.length ? athletes[currentLevel - 1]?.name : "Loading..."}
        </div>
        {!gameOver && athletes.length > 0 && (
          <>
            <input
              ref={guessInputRef}
              type="text"
            ...
            >
              {currentLevel < 5 ? "Next Player" : "Finish"}
            </button>
          </>
        )}
        {gameOver && (
